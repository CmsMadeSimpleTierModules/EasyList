...
$type 				= easylist_utils::init_var('type', $params, null);
$fielddef_id 		= easylist_utils::init_var('fielddef_id', $params, null);
$name 				= easylist_utils::init_var('name', $params, '');
$alias 				= easylist_utils::init_var('alias', $params, '');
$help 				= easylist_utils::init_var('help', $params, '');
$custom_input		= easylist_utils::init_var('custom_input', $params, array());
$extra				= easylist_utils::init_var('extra', $params, ''); // <- (Remove this in 1.5)
...
if(!empty($fielddef_id)) {
	...
	$fielddef_obj = EasyListFielddefOperations::Load($this, 'fielddef_id', $fielddef_id);
} 
// Load fielddef from type
elseif(!empty($type)) {

	$fielddef_obj = EasyListFielddefOperations::LoadFielddefByType($type, $this);
}
...
if (isset($params['submit']) || isset($params['apply']) || isset($params['save_create'])) {
	...
	*/
    ...
    if (empty($errors)) {	
		...
		// check if unique
		$type_exists = EasyListFielddefOperations::TestExistanceByType($this, $fielddef_obj->GetType());
    ...
    if (empty($errors)) {
		...
		// Save field
		EasyListFielddefOperations::Save($this, $fielddef_obj);
...
if(isset($fielddef_obj) && $fielddef_id > 0) {
	...
	$fielddefs = array_merge(array($this->ModLang('select_one')=>null), EasyListFielddefOperations::GetFielddefTypes());
