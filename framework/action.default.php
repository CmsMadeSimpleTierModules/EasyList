...
if(isset($params['template_summary'])) {
	$summarytemplate = trim($params['template_summary']);
}
elseif(isset($params['summarytemplate'])) {
	$summarytemplate = trim($params['summarytemplate']);
}
else {
  $tpl = CmsLayoutTemplate::load_dflt_by_type($this->_GetModuleAlias().'::summary');
    if( !is_object($tpl) ) {
	audit('',$this->GetName(),'Aucun gabarit sommaire par défaut n\'a &eacute;t&eacute; trouv&eacute;');//fr
    return;
  }
  $summarytemplate = $tpl->get_name();
...
$item_query->AppendTo(EasyListQuery::VARTYPE_WHERE, 'A.active = 1');
...
while($result && $row = $result->FetchRow()) {
	...
	if(!isset($this->_item_cache[$row['item_id']])) {
		...
		EasyListItemOperations::Load($this, $cache, $row);	
