...
// functions
//
function easylist_get_category_tree($module_alias,$parent_id = -1)
{
  $db = cmsms()->GetDb();
  ...
  if( is_array($data) && count($data) )
	...
	{
	  $tmp = easylist_get_category_tree($module_alias,$data[$i]['category_id']);
...
function easylist_create_category_flatlist($tree,$parent_id = -1)
{
  $data = array();
  $order = 1;
  foreach( $tree as &$node )
      ...
      if( is_array($node) && count($node) == 2 )
	{
	  ...
	  if( isset($node[1]) && is_array($node[1]) )
	    {
	      $data = array_merge($data,easylist_create_category_flatlist($node[1],$pid));
	    }
	}
      else
	{
...
// Get the data
//
$tree = easylist_get_category_tree($this->_GetModuleAlias());
...
if( isset($params['cancel']) )
...
else if( isset($params['submit']) ) 
  {
    ...
    $flatlist = easylist_create_category_flatlist($orderdata);
