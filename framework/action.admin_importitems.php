...
$separator 			= easylist_utils::init_var('separator', $params, ';'); // <- Default could be option 
$enclouser 			= easylist_utils::init_var('enclouser', $params, '"'); // <- Default could be option 
$import_values 		= easylist_utils::init_var('import_values', $params, array());
...
foreach($item as $k=>$v) {
	...
	if($v instanceof EasyListFielddefArray) {
		...
		$obj->alias = $k;
		$obj->required = in_array($k, EasyListItem::$mandatory) ? 1 : 0;
		$obj->required_image = in_array($k, EasyListItem::$mandatory) ? $admintheme->DisplayImage('icons/system/true.gif', '','','','systemicon') : $admintheme->DisplayImage('icons/system/false.gif','','','','systemicon');
...
if (isset($params['do_import'])) {
	...
	if(empty($errors)) {
		...
		$csv = new EasyListCsvIterator($dest_path, true, $separator, $enclouser);
		...
		foreach($database_values as $item_values) {
			...
			foreach($item_values as $key=>$value) {
				...
				$key 	= utf8_encode($key);
				$value 	= explode(EASYLIST_VALUE_SEPARATOR, utf8_encode($value));
			
				$obj->SetPropertyValue($key, $value);
				if($key == 'item_id')
					EasyListItemOperations::Load($this, $obj); // <- Try loading item if we actually got ID.
			}
...
if (isset($params['submit'])) {
	...
	if(empty($errors) || isset($params['do_import'])) {
		...
		// Initiate CsvIterator
		$csv = new EasyListCsvIterator($dest_path, false, $separator, $enclouser);
		$csv->next();
		$tmp = $csv->current();
		$csv->tearDown();
