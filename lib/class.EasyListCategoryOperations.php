...
class EasylistCategoryOperations
{
	...
	static final public function Save(Easylist &$mod, EasyListCategory &$obj)
	{
		// Check against mandatory list
		foreach(EasyListCategory::$mandatory as $rule) {
	...
	static final public function Delete(Easylist &$mod, EasyListCategory &$obj)
	{
		$db = cmsms()->GetDb();
	...
	static final public function Load(Easylist &$mod, EasyListCategory &$obj, $row = false)
	{	
		$db = cmsms()->GetDb();
		...
		if ($row) {
			...
			// Children
			$query = 'SELECT category_id FROM ' . cms_db_prefix() . 'module_' . $mod->_GetModuleAlias() . '_category WHERE parent_id = ? AND active = 1 ORDER BY position';
	...
	static final public function GetHierarchyList(Easylist &$mod, $null = true, $excludes = null)
	{
	...
	// No in use
	static final public function GetChildrensForCategory(Easylist &$mod, $id)
	{
		$db = cmsms()->GetDb();
	...
	static final public function GetCategoryNameFromId(Easylist &$mod, $id_array)
	{
		$db = cmsms()->GetDb();
	...
	static final public function UpdateHierarchyPosition(Easylist &$mod, $id)
	{
		$db = cmsms()->GetDb();
	...
	static final public function UpdateHierarchyPositions(Easylist &$mod)
	{
		$db = cmsms()->GetDb();
