...
class EasyListSmarty {
	...
	static final public function loader($params, &$smarty)
	{
		...
		if(is_null($instance))
			throw new EasyListException($smarty->left_delimiter . "EasyListLoader" . $smarty->right_delimiter . ": Parameter instance is not given.");
	
		if(is_null($value))
			throw new EasyListException($smarty->left_delimiter . "EasyListLoader" . $smarty->right_delimiter . ": Parameter value is not given.");
			
		// Load wanted instance
		$instance = cmsms()->GetModuleInstance($instance);
		if(!$instance instanceof EASYLIST)
			throw new EasyListException($smarty->left_delimiter . "EasyListLoader" . $smarty->right_delimiter . ": Loaded instance is not ListIt2 instance.");
		...
		switch($item) {
			...
			default:
				throw new EasyListException($smarty->left_delimiter . "EasyListLoader" . $smarty->right_delimiter . ": Unknown item type");
