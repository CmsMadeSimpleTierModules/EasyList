...
class EasyListFielddefOperations
{
	#---------------------
	# Constants
	#---------------------
	...
	static public final function ScanModules()
	{
		...
		array_unshift($installed_modules, EASYLIST);
	...
	static final private function LoadDatabaseInfo($force_db = false, $load_all = false)
	{
		...
		$query = "SELECT * FROM " . cms_db_prefix() . "module_easylist_fielddefs";	
		if(!$load_all)
	...
	static final public function ToggleActive($type)
	{
		...
		$query  = 'SELECT active FROM ' . cms_db_prefix() . 'module_easylist_fielddefs WHERE type=?';
	...
	static final public function SetActiveStatus($type, $status)
	{
		...
		$query  = 'UPDATE ' . cms_db_prefix() . 'module_easylist_fielddefs SET active=? WHERE type=?';
	...
	static final public function ToggleDisabled($type)
	{
		...
		$query  = 'SELECT disabled FROM ' . cms_db_prefix() . 'module_easylist_fielddefs WHERE type=?';
		$active = $db->GetOne($query, array($type));
	...
	static final public function SetDisabledStatus($type, $status)
	{
		...
		$query  = 'UPDATE ' . cms_db_prefix() . 'module_easylist_fielddefs SET disabled=? WHERE type=?';
		$result = $db->Execute($query, array($status, $type));
	...
	static final public function Register($originator, $type, $path, $disabled = 0, $active = 1)
	{
		...
		$query  = 'SELECT originator FROM ' . cms_db_prefix() . 'module_easylist_fielddefs WHERE type=?';
		$exists = $db->GetOne($query, array($type));
		
		if($exists)
			return false;
				
		$query  = 'INSERT INTO ' . cms_db_prefix() . 'module_easylist_fielddefs (type, originator, path, active, disabled) VALUES (?,?,?,?,?)';
	...
	static final public function Unregister($type)
	{
		...
		$query = 'DELETE FROM ' . cms_db_prefix() . 'module_easylist_fielddefs WHERE type = ?';
	...
	static final public function Save(Easylist &$mod, EasyListFielddefBase &$obj)
	{
		$db = cmsms()->GetDb();	
		...
		if ($obj->GetAlias() == '') {
			...
			if($dbresult > 0)
				$generated_alias .= '_'.($dbresult+1);			
			$obj->SetAlias($generated_alias);
		}	
	...
	static final public function Delete(Easylist &$mod, $fielddef_id)
	{
		$db = cmsms()->GetDb();
/*		
...
*/		
	 ...
	 * @return EasyListFielddefBase Object
	 */
	...
	static final public function LoadValuesForFieldDef(Easylist &$mod, EasyListFielddefBase &$obj)
	{			
		$db = cmsms()->GetDb();
	...
	static final public function TestExistanceByType(Easylist &$mod, $type)
	{			
		$db = cmsms()->GetDb();
