...
class EasyListHierarchyManager {
	...
	#--------------------- 	
																		// |- Useless	
	static public final function &FillNodeWithChild(Easylist &$mod, $id, $returnid, &$node, &$nodelist, &$count, &$prevdepth, $origdepth, &$params) 
	{	
		...
		return $node;
																		...
																		// |- Useless
	static public final function &FillNodeWithItem(Easylist &$mod, $id, $returnid, &$node, &$item, &$nodelist, &$count, &$prevdepth, $origdepth, &$params) 
	{		
		// Grap current item
		$current_item = cms_utils::get_app_data('easilist_item');
		...
		return $item;
																	...
																	// |- Useless	
	static public final function GetChildNodes(Easylist &$mod, $id, $returnid, &$id_list, &$nodelist, &$count, &$prevdepth, $origdepth, &$params, &$showparents) 
	{	
		if (!empty($id_list)) {
